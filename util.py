...
def wasserstein(X,t,p,lam=10,its=10,sq=False,backpropT=False):
    ...
    Xc = tf.gather(X,ic)
    Xt = tf.gather(X,it)
    nc = tf.cast(Xc, tf.float32)
    nt = tf.cast(Xt, tf.float32)
    ''' Compute distance matrix'''
    if sq:
        M = pdist2sq(Xt,Xc)
    else:
    ...
    M_drop = tf.keras.layers.Dropout(M,10/(nc*nt))
    ...
    col = tf.stack(tf.constant(0),[delta*tf.ones(tf.shape(M[:,0:1])),tf.zeros((1,1))])
    Mt = tf.stack(tf.constant(0),[M,row])
    Mt = tf.stack(1,[Mt,col])

    ''' Compute marginal vectors '''
    a = tf.stack(tf.constant(0),[p*tf.ones(tf.shape(tf.where(t>0)[:,0:1]))/nt, (1-p)*tf.ones((1,1))])
    b = tf.stack(tf.constant(0),[(1-p)*tf.ones(tf.shape(tf.where(t<1)[:,0:1]))/nc, p*tf.ones((1,1))])
